...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    return JSONResponse(
        ...
        content={"detail": error_detail, "message": "Erro na validação dos dados de entrada"}
    )
...
async def log_requests(request: Request, call_next):
    ...
    try:
        ...
        body_str = body.decode()
        if len(body_str) > 0:
            logger.info(f"Corpo da requisição: {body_str[:1000]}{'...' if len(body_str) > 1000 else ''}")
    ...
    logger.info(f"Requisição processada em {process_time:.2f}s com status {response.status_code}")
...
def health_check():
    ...
    return {"status": "online", "message": "API de análise de sentimentos está operacional"}

@app.post("/analyze")
async def analyze_sentiment(message_list: list[Message]):
    start_time = time.time()
    logger.info(f"Nova requisição de análise recebida com {len(message_list)} mensagens")
    ...
    try:
        ...
        end_time = time.time()
        logger.info(f"Análise concluída com sucesso em {end_time - start_time:.2f} segundos")
        return result
    except Exception as e:
        ...
        return JSONResponse(
            ...
            content={"error": str(e), "message": "Ocorreu um erro ao processar sua solicitação"}
        )
